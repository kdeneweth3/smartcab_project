            ...
            while True:
                try:
                    ...
                    if self.quit or self.env.done:
                        # my info
                        stopdeadline = (self.env.get_deadline(self.env.primary_agent))+1
                        #print 'stopDEADLINE:', stopdeadline
                        startdeadline = float(self.env.primary_agent.initDeadline)
                        #print 'startDEADLINE:', startdeadline
                        trial_num = trial
                        if self.env.result == 'Win':
                            remaining_time = round(1-((startdeadline - stopdeadline)/startdeadline),2)
                        else:
                            remaining_time = 0
                        self.record[trial_num] = tuple([self.env.result,remaining_time])
